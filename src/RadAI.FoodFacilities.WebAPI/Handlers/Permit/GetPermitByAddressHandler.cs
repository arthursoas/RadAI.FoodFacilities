...
{
    public class GetPermitByAddressHandler : HandlerBase, IRequestHandler<GetPermitByAddressRequest, ResponseBase<GetPermitResponse[]>>
    {
        private readonly IPermitManager _permitManager;

        public GetPermitByAddressHandler(
            IPermitManager permitManager)
        {
            _permitManager = permitManager;
        }

        public async Task<ResponseBase<GetPermitResponse[]>> Handle(GetPermitByAddressRequest request, CancellationToken cancellationToken)
        {
